<?php

return [
    'seeders' => [
        'attributes' => [
            'leads' => [
                'description'         => 'Beschrijving',
                'expected-close-date' => 'Verwachte sluitingsdatum',
                'lead-value'          => 'Leadwaarde',
                'sales-owner'         => 'Verkoper',
                'source'              => 'Bron',
                'title'               => 'Titel',
                'type'                => 'Type',
                'pipeline'            => 'Pipeline',
                'stage'               => 'Fase',
            ],

            'persons' => [
                'contact-numbers' => 'Contactnummers',
                'emails'          => 'E-mails',
                'job-title'       => 'Functietitel',
                'name'            => 'Naam',
                'organization'    => 'Organisatie',
                'sales-owner'     => 'Verkoper',
            ],

            'organizations' => [
                'address'     => 'Adres',
                'name'        => 'Naam',
                'sales-owner' => 'Verkoper',
            ],

            'products' => [
                'description' => 'Beschrijving',
                'name'        => 'Naam',
                'price'       => 'Prijs',
                'quantity'    => 'Aantal',
                'sku'         => 'Artikelnummer',
            ],

            'quotes' => [
                'adjustment-amount' => 'Aanpassingsbedrag',
                'billing-address'   => 'Factuuradres',
                'description'       => 'Beschrijving',
                'discount-amount'   => 'Korting bedrag',
                'discount-percent'  => 'Kortingspercentage',
                'expired-at'        => 'Verloopt op',
                'grand-total'       => 'Eindtotaal',
                'person'            => 'Persoon',
                'sales-owner'       => 'Verkoper',
                'shipping-address'  => 'Verzendadres',
                'sub-total'         => 'Subtotaal',
                'subject'           => 'Onderwerp',
                'tax-amount'        => 'Belastingbedrag',
            ],

            'warehouses' => [
                'contact-address' => 'Contactadres',
                'contact-emails'  => 'Contacte-mails',
                'contact-name'    => 'Contactpersoon',
                'contact-numbers' => 'Contactnummers',
                'description'     => 'Beschrijving',
                'name'            => 'Naam',
            ],
        ],

        'email' => [
            'activity-created'      => 'Activiteit aangemaakt',
            'activity-modified'     => 'Activiteit gewijzigd',
            'date'                  => 'Datum',
            'new-activity'          => 'Je hebt een nieuwe activiteit, zie de details hieronder',
            'new-activity-modified' => 'Je activiteit is gewijzigd, zie de details hieronder',
            'participants'          => 'Deelnemers',
            'title'                 => 'Titel',
            'type'                  => 'Type',
        ],

        'lead' => [
            'pipeline' => [
                'default' => 'Standaard Pipeline',

                'pipeline-stages' => [
                    'follow-up'   => 'Opvolging',
                    'lost'        => 'Verloren',
                    'negotiation' => 'Onderhandeling',
                    'new'         => 'Nieuw',
                    'prospect'    => 'Prospect',
                    'won'         => 'Gewonnen',
                ],
            ],

            'source' => [
                'direct'   => 'Direct',
                'email'    => 'E-mail',
                'phone'    => 'Telefoon',
                'web'      => 'Website',
                'web-form' => 'Webformulier',
            ],

            'type' => [
                'existing-business' => 'Bestaande Klant',
                'new-business'      => 'Nieuwe Klant',
            ],
        ],

        'user' => [
            'role' => [
                'administrator-role' => 'Beheerdersrol',
                'administrator'      => 'Beheerder',
            ],
        ],

        'workflow' => [
            'email-to-participants-after-activity-updation' => 'E-mails naar deelnemers na aanpassing activiteit',
            'email-to-participants-after-activity-creation' => 'E-mails naar deelnemers na aanmaak activiteit',
        ],
    ],

    'installer' => [
        'index' => [
            'create-administrator' => [
                'admin'            => 'Beheerder',
                'krayin'           => 'Krayin',
                'confirm-password' => 'Bevestig wachtwoord',
                'email'            => 'E-mail',
                'email-address'    => 'admin@example.com',
                'password'         => 'Wachtwoord',
                'title'            => 'Maak Beheerder',
            ],

            'environment-configuration' => [
                'algerian-dinar'              => 'Algerijnse Dinar (DZD)',
                'allowed-currencies'          => 'Toegestane valuta\'s',
                'allowed-locales'             => 'Toegestane talen',
                'application-name'            => 'Applicatienaam',
                'argentine-peso'              => 'Argentijnse Peso (ARS)',
                'australian-dollar'           => 'Australische Dollar (AUD)',
                'krayin'                      => 'Krayin',
                'bangladeshi-taka'            => 'Bengalese Taka (BDT)',
                'brazilian-real'              => 'Braziliaanse Real (BRL)',
                'british-pound-sterling'      => 'Britse Pond (GBP)',
                'canadian-dollar'             => 'Canadese Dollar (CAD)',
                'cfa-franc-bceao'             => 'CFA-frank BCEAO (XOF)',
                'cfa-franc-beac'              => 'CFA-frank BEAC (XAF)',
                'chilean-peso'                => 'Chileense Peso (CLP)',
                'chinese-yuan'                => 'Chinese Yuan (CNY)',
                'colombian-peso'              => 'Colombiaanse Peso (COP)',
                'czech-koruna'                => 'Tsjechische Kroon (CZK)',
                'danish-krone'                => 'Deense Kroon (DKK)',
                'database-connection'         => 'Databaseverbinding',
                'database-hostname'           => 'Databasehost',
                'database-name'               => 'Databasenaam',
                'database-password'           => 'Databasewachtwoord',
                'database-port'               => 'Databasepoort',
                'database-prefix'             => 'Databasevoorvoegsel',
                'database-username'           => 'Databasegebruiker',
                'default-currency'            => 'Standaardvaluta',
                'default-locale'              => 'Standaardtaal',
                'default-timezone'            => 'Standaardtijdzone',
                'default-url'                 => 'Standaard URL',
                'default-url-link'            => 'https://localhost',
                'egyptian-pound'              => 'Egyptisch Pond (EGP)',
                'euro'                        => 'Euro (EUR)',
                'fijian-dollar'               => 'Fiji Dollar (FJD)',
                'hong-kong-dollar'            => 'Hongkong Dollar (HKD)',
                'hungarian-forint'            => 'Hongaarse Forint (HUF)',
                'indian-rupee'                => 'Indiase Roepie (INR)',
                'indonesian-rupiah'           => 'Indonesische Rupiah (IDR)',
                'israeli-new-shekel'          => 'Israëlische Shekel (ILS)',
                'japanese-yen'                => 'Japanse Yen (JPY)',
                'jordanian-dinar'             => 'Jordaanse Dinar (JOD)',
                'kazakhstani-tenge'           => 'Kazachse Tenge (KZT)',
                'kuwaiti-dinar'               => 'Koeweitse Dinar (KWD)',
                'lebanese-pound'              => 'Libanees Pond (LBP)',
                'libyan-dinar'                => 'Libische Dinar (LYD)',
                'malaysian-ringgit'           => 'Maleisische Ringgit (MYR)',
                'mauritian-rupee'             => 'Mauritiaanse Roepie (MUR)',
                'mexican-peso'                => 'Mexicaanse Peso (MXN)',
                'moroccan-dirham'             => 'Marokkaanse Dirham (MAD)',
                'mysql'                       => 'MySQL',
                'nepalese-rupee'              => 'Nepalese Roepie (NPR)',
                'new-taiwan-dollar'           => 'Nieuwe Taiwanese Dollar (TWD)',
                'new-zealand-dollar'          => 'Nieuw-Zeelandse Dollar (NZD)',
                'nigerian-naira'              => 'Nigeriaanse Naira (NGN)',
                'norwegian-krone'             => 'Noorse Kroon (NOK)',
                'omani-rial'                  => 'Omaanse Rial (OMR)',
                'pakistani-rupee'             => 'Pakistaanse Roepie (PKR)',
                'panamanian-balboa'           => 'Panamese Balboa (PAB)',
                'paraguayan-guarani'          => 'Paraguayaanse Guarani (PYG)',
                'peruvian-nuevo-sol'          => 'Peruaanse Sol (PEN)',
                'pgsql'                       => 'pgSQL',
                'philippine-peso'             => 'Filipijnse Peso (PHP)',
                'polish-zloty'                => 'Poolse Zloty (PLN)',
                'qatari-rial'                 => 'Qatarese Rial (QAR)',
                'romanian-leu'                => 'Roemeense Leu (RON)',
                'russian-ruble'               => 'Russische Roebel (RUB)',
                'saudi-riyal'                 => 'Saoedi-Riyal (SAR)',
                'select-timezone'             => 'Selecteer Tijdzone',
                'singapore-dollar'            => 'Singapore Dollar (SGD)',
                'south-african-rand'          => 'Zuid-Afrikaanse Rand (ZAR)',
                'south-korean-won'            => 'Zuid-Koreaanse Won (KRW)',
                'sqlsrv'                      => 'SQLSRV',
                'sri-lankan-rupee'            => 'Sri Lankaanse Roepie (LKR)',
                'swedish-krona'               => 'Zweedse Kroon (SEK)',
                'swiss-franc'                 => 'Zwitserse Frank (CHF)',
                'thai-baht'                   => 'Thaise Baht (THB)',
                'title'                       => 'Winkelconfiguratie',
                'tunisian-dinar'              => 'Tunesische Dinar (TND)',
                'turkish-lira'                => 'Turkse Lira (TRY)',
                'ukrainian-hryvnia'           => 'Oekraïense Hryvnia (UAH)',
                'united-arab-emirates-dirham' => 'VAE Dirham (AED)',
                'united-states-dollar'        => 'Amerikaanse Dollar (USD)',
                'uzbekistani-som'             => 'Oezbeekse Som (UZS)',
                'venezuelan-bolívar'          => 'Venezolaanse Bolívar (VEF)',
                'vietnamese-dong'             => 'Vietnamese Dong (VND)',
                'warning-message'             => 'Let op! De instellingen voor je standaardtaal en standaardvaluta kunnen later niet meer worden gewijzigd.',
                'zambian-kwacha'              => 'Zambiaanse Kwacha (ZMW)',
            ],

            'installation-processing' => [
                'krayin'       => 'Krayin Installatie',
                'krayin-info'  => 'Database tabellen worden aangemaakt, dit kan even duren',
                'title'        => 'Installatie',
            ],

            'installation-completed' => [
                'admin-panel'                => 'Beheerpaneel',
                'krayin-forums'              => 'Krayin Forum',
                'customer-panel'             => 'Klantpaneel',
                'explore-krayin-extensions'  => 'Bekijk Krayin Extensies',
                'title'                      => 'Installatie Voltooid',
                'title-info'                 => 'Krayin is succesvol geïnstalleerd.',
            ],

            'ready-for-installation' => [
                'create-databsae-table'   => 'Maak database tabellen',
                'install'                 => 'Installeren',
                'install-info'            => 'Krayin Installatieproces',
                'install-info-button'     => 'Klik op de onderstaande knop om verder te gaan',
                'populate-database-table' => 'Database vullen',
                'start-installation'      => 'Start Installatie',
                'title'                   => 'Klaar voor Installatie',
            ],

            'start' => [
                'locale'        => 'Taal',
                'main'          => 'Start',
                'select-locale' => 'Selecteer Taal',
                'title'         => 'Krayin Installatie',
                'welcome-title' => 'Welkom bij Krayin',
            ],

            'server-requirements' => [
                'calendar'    => 'Kalender',
                'ctype'       => 'cType',
                'curl'        => 'cURL',
                'dom'         => 'DOM',
                'fileinfo'    => 'Bestandsinfo',
                'filter'      => 'Filter',
                'gd'          => 'GD',
                'hash'        => 'Hash',
                'intl'        => 'Intl',
                'json'        => 'JSON',
                'mbstring'    => 'mbstring',
                'openssl'     => 'openssl',
                'pcre'        => 'PCRE',
                'pdo'         => 'PDO',
                'php'         => 'PHP',
                'php-version' => '8.1 of hoger',
                'session'     => 'Sessie',
                'title'       => 'Systeemvereisten',
                'tokenizer'   => 'Tokenizer',
                'xml'         => 'XML',
            ],

            'back'                     => 'Terug',
            'krayin'                   => 'Krayin',
            'krayin-info'              => 'een Community Project van',
            'krayin-logo'              => 'Krayin Logo',
            'continue'                 => 'Doorgaan',
            'installation-description' => 'De installatie van Krayin omvat een aantal stappen. Hieronder een overzicht van het proces:',
            'installation-info'        => 'We zijn blij dat je hier bent!',
            'installation-title'       => 'Welkom bij de Installatie',
            'installation-wizard'      => 'Installatiewizard taal',
            'title'                    => 'Krayin Installer',
            'webkul'                   => 'Webkul',
        ],
    ],
];
